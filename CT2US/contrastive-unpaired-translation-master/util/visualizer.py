...
class Visualizer():
    ...
    def save_all_images(self,visuals,n):

        for label, image in visuals.items():
            image_numpy = util.tensor2im(image)
            img_name = 'epoch_{}_{}.png'.format(n,label)
            if (label=='fake_B'):
                util.save_image(image_numpy, os.path.join("train_set/fake_b", img_name))
            if (label=='real_B'):
                util.save_image(image_numpy, os.path.join("train_set/real_b", img_name))
